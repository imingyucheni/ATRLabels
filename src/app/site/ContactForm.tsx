...
export default function ContactForm({ volumes }: { volumes: string[] }) {
  const t = useT();
  const tMsg = useTMsg();
  ...
  return (
    <form action={action} className="site-apply-form">
      ...
      <label className="f">{t("想了解什么？（常用渠道、发货量等）")}<textarea name="note" rows={3} maxLength={500} defaultValue={state?.values?.note} /></label>
      {/* 防机器人：正常人看不到这个输入框 */}
