...
export default function SelectPrint({
  ...
  const [paper, setPaper] = useState<PaperSize | undefined>(defaultPaper);
    ...
    <div className="card table-wrap">
      <div className="row" style={{ marginBottom: 8 }}>
        <a
          ...
          href={sel.size ? `/api/labels/merge?ids=${[...sel].join(",")}${paper ? `&paper=${paper}` : ""}` : undefined}
          target="_blank"
        ...
        {defaultPaper && (
          <select value={paper} onChange={(e) => setPaper(e.target.value as PaperSize)} aria-label={t("纸张")} style={{ width: "auto" }}>
            {PAPER_SIZES.map((p) => (
