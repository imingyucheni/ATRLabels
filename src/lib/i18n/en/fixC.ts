export const fixC: Record<string, string> = {
  "收件地址核对": "Address check",
  "下单时地址核对": "Address check at order time",
  ...
  "服务商": "Provider",
  "Google（每月前 5000 次免费）": "Google (first 5,000/month free)",
  "USPS（需签约，按月收费）": "USPS (contract, monthly fee)",
  "Google Cloud → 凭据里的 API 密钥": "API key from Google Cloud → Credentials",
  "每月最多查询次数": "Monthly lookup limit",
  "到了上限本月就不再查，不会产生费用；0 = 不限制": "Checks stop for the month at the limit, so no charges; 0 = unlimited",
  "USPS 账号（选 USPS 时才用）": "USPS account (only used when USPS is selected)",
  "已保存（地址核对已停用）": "Saved (address check disabled)",
  "Google 地址核对连接成功": "Google address check connected",
  "请先填写 Google API Key": "Please enter the Google API key first",
