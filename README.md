...
- **官方账单补差（多退少补）**：ShipBest 系统扣的是预报价格，官方账单出来后，重量或分区有差异会多退少补。在“补差导入”上传他们给的表格（.xlsx / .csv）：
  ...
  - 向客户补收或退还多少，按设置里的转嫁规则计算。默认**按该单下单时的加价比例**：例如服务商补收 0.06、该客户加价 5%，客户补 0.06 × 1.05 = 0.063 → **向上取到 0.07**（退款则舍去零头）。只按百分比，不再加每单固定加价和最低利润。也可以改成按原金额转嫁或不转嫁；
  - 同一张表（按内容判断，重新另存也能识别）不能重复导入；某张面单已经有相同金额的补差时会提示可能重复；
  - 没对上的行也会保存，之后可以输入单号手动关联；整个批次可以撤销。
  - **按客户导出补差明细**发给客户：每个客户一份 CSV，只含他自己的单，保留日期、州、尺寸、结算/预报重量、分区、备注，金额为向客户补收/退还的数；不导出应收/实收、各项费用、我们在服务商的账户名等内部信息。
